...
void	mat_type_chk2(t_scene *scene2, int i)
{
	if (scene2->objects[i]->type == Sphere)
	{
		if (!((scene2->objects[i]->mat_type == 1)
				|| (scene2->objects[i]->mat_type == 2)
				|| (scene2->objects[i]->mat_type == 3)
				|| (scene2->objects[i]->mat_type == 4)))
			ft_exit(-1, "Error\nSphere mat_type error\n");
	}
	else if (scene2->objects[i]->type == Plane)
	{
		if (!((scene2->objects[i]->mat_type == 1)
				|| (scene2->objects[i]->mat_type == 5)))
			ft_exit(-1, "Error\nPlane mat_type error\n");
	}
}
...
void	mat_type_chk(t_scene *scene2)
{
	...
	{
		if ((scene2->objects[i]->type == Sphere)
			|| (scene2->objects[i]->type == Plane))
		{
			mat_type_chk2(scene2, i);
		}
		else if (scene2->objects[i]->type == Cylinder)
		{
			if (!(scene2->objects[i]->mat_type == 1))
				ft_exit(-1, "Error\nCylinder mat_type error\n");
		}
